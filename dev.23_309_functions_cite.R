...
normalize_rna_data <- function(ObjList) {
  ObjList <- lapply(ObjList, SCTransform)
  
  Obj <- merge(ObjList[[1]], ObjList[2:length(ObjList)])
  
  VariableFeatures(Obj) <- rownames(Obj[["RNA"]])
  Obj <- RunPCA(Obj)
  Obj <- FindNeighbors(Obj, dims = 1:30)
  Obj <- FindClusters(Obj, resolution = 0.8, algorithm=3, verbose = FALSE)
  
 # Obj$monaco_ann1 <- monaco_ann1(Obj)
 # Obj$monaco_ann2 <- monaco_ann2(Obj)
  
  Obj <- RunUMAP(Obj, reduction = 'pca', dims = 1:10, assay = 'RNA', 
...
for(i in 1:length(lanes)){
  ...
  SeuratObj[[i]][["CITE"]] <- CreateAssayObject(counts = B1_US_data[[i]]$`Antibody Capture`[1:165,colnames(SeuratObj[[i]])])
  SeuratObj[[i]][["HTO"]] <- CreateAssayObject(counts= B1_US_data[[i]]$`Custom`[1:5,colnames(SeuratObj[[i]])])

  SeuratObj[[i]] <- RenameCells(SeuratObj[[i]],
                                new.names = paste(substr(colnames(SeuratObj[[i]]),
                                                         start = 1, stop = 17),lanes[[i]],"_23-309", sep = ""))
...
B1_US_merge <- PercentageFeatureSet(B1_US_merge, pattern = "^MT-",col.name = "percent.mito")
 ...
 # B1_US_merge2<- JoinLayers(B1_US_merge, overwrite = TRUE)  
  ...
  ggsave(paste0(save_name,"_postfiltering.pdf"), plot =VlnPlot(B1_US_merge, c( "percent.mito","percent_ribo","percent_hb", "nFeature_RNA", "nCount_RNA"), group.by = "Lane", pt.size=0, ncol = 3)) + theme(axis.text.x = element_text(angle = 45, hjust=1))
...
runTransferLearningBM <- function(Obj) {
...
#                      destfile = paste0(projection_path, 'BoneMarrow_RefMap_uwot_model.uwot'))
  ...
  query<- Obj # subset(Obj, subset = Lane %in% c(7, 8, 9))
